...
trace_data_wrangling = function(res, sim_type){
  ...
  dat[,3] = rep(1:1000, each = 5)
  dat[,4] = rep("est", times = 5000)
  dat[,5] = rep(sim_type, times = 5000)
  
  true_dat = as.data.frame(matrix(rep(theta[P], each = 1000), nrow = 1000, ncol = 5))
  true_dat = true_dat %>% pivot_longer(cols = everything(), names_to = "Variable", values_to = "Value")
  true_dat[,3] = rep(1:1000, each = 5)
  true_dat[,4] = rep("true", times = 5000)
  true_dat[,5] = rep(sim_type, times = 5000)
...
ggplot_elo_var = function(res_list, alg_type){
  #wrangling
  x = res_list[[1]]$true
