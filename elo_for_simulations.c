...
void elo_inflation2(double*K,
                    ...
                    int*adaptive,
                    double*A,double*B,
                    double*cumsum,double*exp_bias,double*exp_bias_item,double*mD){
     int x=0;
     double e=0;
     double L=0;
     double p=0;
     ...
     for(int g=0;g<games[0];g++){
        for(int r=0;r<reps[0];r++){
            for(int i=0;i<N[0];i++){
                ...
                if(adaptive[0]==1){
                  for(int s=0;s<M[0];s++){
                    exp_bias[i+g*N[0]]=exp_bias[i+g*N[0]]+1.00*(cumsum[s+1]-cumsum[s])/Mp*(d[s+r*M[0]]-delta[s])/reps[0];
                  }
                }
                if(adaptive[0]==0){
                  for(int s=0;s<M[0];s++){
                    exp_bias[i+g*N[0]]=exp_bias[i+g*N[0]]+(d[s+r*M[0]]-delta[s])/(M[0]*reps[0]);
                  }
                }
                ...
                exp_bias_item[j+r*M[0]+g*reps[0]*M[0]]=t[i+r*N[0]]-theta[i];
...
void elo_true(double*K,int*reps,int*games,int*N,int*M,double*theta,double*delta,double*t,double*d,double*mT,double*vT,double*A,double*B,double*cumsum,double*mD){
  int x=0;
  double e=0;
  double L=0;
  double p=0;
...
void elo_autocorrelation(double*K,int*reps,int*games,int*N,int*M,double*theta,double*delta,double*t,double*d,double*mT,double*vT,double*Theta,double*Mean_delta,double*cumsum,double*mD){
  int x=0;
  double e=0;
  double L=0;
  double p=0;
  ...
  for(int g=0;g<games[0];g++){
    for(int r=0;r<reps[0];r++){
      for(int i=0;i<N[0];i++){
        ...
        /*add to the mean of person i at time point g*/
        mT[i+g*N[0]]=mT[i+g*N[0]]+t[i+r*N[0]]/reps[0];
        Theta[i+r*N[0]+g*reps[0]*N[0]]=t[i+r*N[0]];
      }
      /*for each item, add the current value to the mean of item j at time point g*/
      for(int j=0;j<M[0];j++){
        mD[j+g*M[0]]=mD[j+g*M[0]]+d[j+r*M[0]]/reps[0];
        Mean_delta[r+g*reps[0]]=Mean_delta[r+g*reps[0]]+d[j+r*M[0]]/M[0];
      }
    }
    /*compute the variances of the item and person ratings at time point g*/
    for(int r=0;r<reps[0];r++){
...
void elo_history(double*K,int*reps,int*games,int*N,int*M,double*theta,double*delta,double*t,double*d,double*t_hist,double*d_hist,double*mT,double*vT,double*A,double*B,double*cumsum,int*use_mean,int*LH,double*mD,int*Items_total,int*Items_count){
     int x=0;
     double e=0;
     double L=0;
     double p=0;
     ...
     for(int g=0;g<games[0];g++){
        ...
        for(int r=0;r<reps[0];r++){
            for(int i=0;i<N[0];i++){
                ...
                g_min=LH[0]+1;
                for(int s=0;s<M[0];s++){
                  if(Items_total[s+r*M[0]]<g_min){
                    g_min=Items_total[s+r*M[0]];
                  }
                }
                for(int s=0;s<M[0];s++){
                    if((g<LH[0])|(g_min<LH[0])){
                            L=1.00;
                    }
                    if((g>(LH[0]-1))&(g_min>(LH[0]-1))){
                        /*if(use_mean[0]==1){
                            avD=0;
                            for(int h=0;h<LH[0];h++){
                        ...
                        if(use_mean[0]==0){
                            dif=t_hist[i+r*N[0]+(gg-1)*N[0]*reps[0]]-d_hist[s+r*M[0]+(Items_count[s+r*M[0]])*M[0]*reps[0]];
                        }
                        L=exp(dif*dif*A[0]+dif*B[0]);
                    }
                    Mp=Mp+L;
                ...
                Items_total[j+r*M[0]]=Items_total[j+r*M[0]]+1;
                Items_count[j+r*M[0]]=Items_count[j+r*M[0]]+1;
                /* compute the true probability correct*/
                L=1/(1+exp((delta[j]-theta[i]))); 
                /*generate the observed response*/
                p=runif(0,1);
                ...
                d_hist[j+r*M[0]+(Items_count[j+r*M[0]]-1)*M[0]*reps[0]]=d[j+r*M[0]];
                if(Items_count[j+r*M[0]]==LH[0]){Items_count[j+r*M[0]]=0;}
            }
            /*for each item, add the current value to the mean of item j at time point g*/
            for(int j=0;j<M[0];j++){
              mD[j+g*M[0]]=mD[j+g*M[0]]+d[j+r*M[0]]/reps[0];
...
void elo_history2(double*K,int*reps,int*games,int*N,int*M,double*theta,double*delta,double*t,double*d,double*t_hist,double*d_hist,double*mT,double*vT,double*A,double*B,double*cumsum,int*use_mean,int*LH,double*mD,int*Items_total,int*Items_count){
  int x=0;
  double e=0;
  double L=0;
  double p=0;
      ...
      for(int i=0;i<N[0];i++){
        ...
        for(int s=0;s<M[0];s++){
          ...
          if((g>(LH[0]-1))&(g_min>(LH[0]-1))){
            ...
            if(use_mean[0]==0){
              dif=t[i+r*N[0]+(gg-1)*N[0]*reps[0]]-d_hist[s+r*M[0]+(Items_count[s+r*M[0]])*M[0]*reps[0]];
            }
            L=exp(dif*dif*A[0]+dif*B[0]);
          }
          Mp=Mp+L;
